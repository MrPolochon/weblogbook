-- ============================================================
-- MIGRATION COMPLÈTE FELITZ BANK & COMPAGNIES - V2
...
-- ===========================================
-- PARTIE 2: NOUVELLES TABLES (sans FK circulaires)
-- ===========================================
...
-- ===========================================
-- PARTIE 3: MODIFICATIONS PLANS_VOL
-- ===========================================
...
-- Fonction pour générer VBAN compagnie
CREATE OR REPLACE FUNCTION set_company_vban_func() RETURNS TRIGGER AS $$
DECLARE
  new_vban TEXT;
BEGIN
...
CREATE TRIGGER set_company_vban
  ...
  FOR EACH ROW
  EXECUTE FUNCTION set_company_vban_func();
...
-- ===========================================
-- PARTIE 6: CRÉER COMPTES POUR EXISTANTS
-- ===========================================
...
DECLARE
  ...
  new_vban TEXT;
BEGIN
  FOR rec IN SELECT id FROM public.profiles WHERE NOT EXISTS (
    SELECT 1 FROM public.felitz_comptes WHERE proprietaire_id = profiles.id AND type = 'personnel'
  ) LOOP
    LOOP
      new_vban := 'MIXOU' || upper(substr(md5(random()::text || clock_timestamp()::text), 1, 22));
      EXIT WHEN NOT EXISTS (SELECT 1 FROM public.felitz_comptes WHERE vban = new_vban);
    END LOOP;
    INSERT INTO public.felitz_comptes (type, proprietaire_id, vban, solde)
    VALUES ('personnel', rec.id, new_vban, 0)
...
DECLARE
  ...
  new_vban TEXT;
BEGIN
  FOR rec IN SELECT id FROM public.compagnies WHERE vban IS NULL LOOP
    LOOP
      new_vban := 'ENTERMIXOU' || upper(substr(md5(random()::text || clock_timestamp()::text), 1, 16));
      EXIT WHEN NOT EXISTS (SELECT 1 FROM public.compagnies WHERE vban = new_vban)
                AND NOT EXISTS (SELECT 1 FROM public.felitz_comptes WHERE vban = new_vban);
    END LOOP;
    UPDATE public.compagnies SET vban = new_vban WHERE id = rec.id;
  END LOOP;
END;
$$;
...
DECLARE
  rec RECORD;
BEGIN
  FOR rec IN SELECT id, vban FROM public.compagnies WHERE vban IS NOT NULL LOOP
    IF NOT EXISTS (SELECT 1 FROM public.felitz_comptes WHERE compagnie_id = rec.id AND type = 'entreprise') THEN
      INSERT INTO public.felitz_comptes (type, compagnie_id, vban, solde)
      VALUES ('entreprise', rec.id, rec.vban, 0)
      ON CONFLICT DO NOTHING;
    END IF;
  END LOOP;
END;
