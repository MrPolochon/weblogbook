...
export default async function AtcPage() {
  ...
  const [{ data: session }, { data: plansChezMoi }] = await Promise.all([
    ...
    admin.from('plans_vol').select('id, numero_vol, aeroport_depart, aeroport_arrivee, statut').eq('current_holder_user_id', user.id).is('pending_transfer_aeroport', null).in('statut', ['en_attente', 'en_cours', 'accepte', 'en_attente_cloture']),
  ]);
