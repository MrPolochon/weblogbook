...
function getFileIcon(name: string) {
  ...
  if (['doc', 'docx', 'txt', 'rtf', 'odt'].includes(ext)) return <FileText className="h-5 w-5 text-red-500" />;
      ...
      {!sections || sections.length === 0 ? (
        ...
        <div className="space-y-5">
          {sections.map((s) => {
            ...
            return (
              <div key={s.id} className="rounded-xl border border-red-200 bg-white overflow-hidden shadow-sm">
                <div className="flex items-center gap-3 px-5 py-3 bg-red-50 border-b border-red-200">
                  ...
                  <h2 className="font-semibold text-red-800">{s.nom}</h2>
                    ...
                    {files.map((f) => (
                      <li key={f.id}>
                        <Link
                          ...
                          className="flex items-center gap-3 px-5 py-3 hover:bg-red-50 transition-colors group"
                        >
                          {getFileIcon(f.nom_original)}
                          <span className="text-sm text-slate-700 font-medium flex-1 truncate group-hover:text-red-700 transition-colors">
                            {f.nom_original}
                          </span>
                          {f.taille_bytes ? (
                            <span className="text-xs text-slate-400 shrink-0">{formatSize(f.taille_bytes)}</span>
                          ) : null}
                          <Download className="h-4 w-4 text-slate-300 group-hover:text-red-600 transition-colors shrink-0" />
                        </Link>
                      </li>
                    ))}
                  </ul>
