...
const CHEQUES_TYPES_GC = ['cheque_salaire'];

export default function MessagerieGroundClient({ messagesRecus, messagesEnvoyes, utilisateurs, currentUserIdentifiant, isAdmin = false }: Props) {
  ...
  const cheques = messagesRecus.filter(m => CHEQUES_TYPES_GC.includes(m.type_message));
  const messagesNormaux = messagesRecus.filter(m => !CHEQUES_TYPES_GC.includes(m.type_message));
  ...
  const tabs = [
    { id: 'cheques', label: 'Chèques salaire', icon: CreditCard, count: cheques.filter(m => !m.cheque_encaisse).length },
        ...
        {activeTab === 'cheques' && cheques.filter(c => !c.cheque_encaisse).length >= 2 && (
          ...
          >
            {encaisserToutLoading ? (
              ...
              <><CheckCheck className="h-4 w-4" />Tout encaisser ({cheques.filter(c => !c.cheque_encaisse).length} chèques)</>
            )}
          </button>
        )}
          ...
          <div className="rounded-xl border border-emerald-300 bg-emerald-50 p-4 space-y-3">
            <div className="flex items-center justify-between">
              <h3 className="font-bold text-emerald-700 flex items-center gap-2">
                ...
                {encaisserToutRecap.nb_cheques} chèque{encaisserToutRecap.nb_cheques > 1 ? 's' : ''} encaissé{encaisserToutRecap.nb_cheques > 1 ? 's' : ''}
              </h3>
            ...
            <div className="space-y-1">
              {encaisserToutRecap.par_compte.map((c, i) => (
                <div key={i} className="flex justify-between text-sm">
                  <span className="text-slate-600">{c.label} ({c.nb} chèque{c.nb > 1 ? 's' : ''})</span>
            ...
            <div className="pt-2 border-t border-emerald-200 flex justify-between text-sm font-bold">
              <span className="text-slate-700">Total net encaissé</span>
          ...
          {activeTab === 'compose' ? (
            <form onSubmit={handleSendMessage} className="p-4 space-y-4">
                  ...
                  <div className="flex items-center gap-1 p-0.5 rounded-lg bg-slate-100 border border-slate-200">
                    <button type="button" onClick={() => setComposeMode('individuel')}
                      className={`flex items-center gap-1.5 px-2.5 py-1 rounded text-xs font-medium transition-colors ${
                        composeMode === 'individuel' ? 'bg-emerald-600 text-white' : 'text-slate-500 hover:text-slate-700'
                      }`}>
                    ...
                    <button type="button" onClick={() => setComposeMode('diffusion')}
                      className={`flex items-center gap-1.5 px-2.5 py-1 rounded text-xs font-medium transition-colors ${
                        composeMode === 'diffusion' ? 'bg-amber-600 text-white' : 'text-slate-500 hover:text-slate-700'
                      }`}>
              ...
              {isAdmin && composeMode === 'diffusion' ? (
                <div>
                  <label className="block text-sm text-slate-600 mb-2 flex items-center gap-1.5">
              ...
              <button
                ...
                className={`w-full flex items-center justify-center gap-2 px-4 py-2 rounded-lg text-white font-medium transition-colors disabled:opacity-50 ${
                  isAdmin && composeMode === 'diffusion' ? 'bg-amber-600 hover:bg-amber-700' : 'bg-emerald-600 hover:bg-emerald-700'
                }`}
              >
      ...
      <div className="lg:col-span-2">
        {selectedMessage ? (
          <div className="rounded-xl border border-slate-200 bg-white shadow-sm overflow-hidden">
            ...
            <div className="p-6">
              {CHEQUES_TYPES_GC.includes(selectedMessage.type_message) && selectedMessage.cheque_montant ? (
