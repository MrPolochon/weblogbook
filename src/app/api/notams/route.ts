...
export async function GET() {
  try {
    ...
    const { data, error } = await supabase
      .from('notams')
      .select('id, identifiant, code_aeroport, du_at, au_at, permanent, champ_a, champ_e, champ_d, champ_q, priorite, reference_fr, annule, created_at')
...
export async function POST(request: Request) {
  try {
    ...
    const { code_aeroport, du_at, au_at, permanent, champ_a, champ_e, champ_d, champ_q, priorite, reference_fr } = body;
    ...
    if (!isPermanent && (!au_at || typeof au_at !== 'string')) return NextResponse.json({ error: 'Date et heure de fin (AU) requises.' }, { status: 400 });
    ...
    const au = isPermanent ? new Date('9999-12-31T23:59:59.999Z') : new Date(au_at);
    ...
    if (!isPermanent && au.getTime() <= du.getTime()) return NextResponse.json({ error: 'La date de fin (AU) doit être après la date de début (DU).' }, { status: 400 });
