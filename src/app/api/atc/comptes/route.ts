...
export async function POST(request: Request) {
  try {
    ...
    const { identifiant, password, grade_id, aussi_pilote } = body;
    ...
    const role = aussi_pilote ? 'pilote' : 'atc';
    ...
    if (createErr) {
      if (createErr.message?.includes('already been registered')) return NextResponse.json({ error: 'Cet identifiant est déjà utilisé.' }, { status: 400 });
    ...
    const { error: profileErr } = await admin.from('profiles').insert({
      ...
      role,
      atc: true,
      atc_grade_id: grade_id && typeof grade_id === 'string' ? grade_id : null,
      armee: false,
    ...
    console.error('ATC comptes POST:', e);
