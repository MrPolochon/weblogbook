...
export async function POST() {
  ...
  if (!instanceId) {
    return NextResponse.json(
      {
        error: 'Démarrez d\'abord un ATIS pour basculer le mode bilingue.',
      },
      { status: 409 }
    );
  }

  const result = await fetchAtisBot<{ ok: boolean; bilingual_mode: boolean }>(
    '/webhook/toggle-bilingual',
