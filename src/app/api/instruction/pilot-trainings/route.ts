...
export async function GET() {
  try {
    ...
    console.error('instruction/pilot-trainings GET:', e);
...
export async function POST(request: Request) {
  try {
    ...
    if (!isExamRequestLicence(licenceCode) || trainingSideForExamLicence(licenceCode) !== 'pilot') {
      return NextResponse.json(
        {
          error: `Licence de training invalide. Choisissez une licence d'examen vol (ex. COM 1, CAT 1…) parmi : ${examLicencesForTrainingSide('pilot').join(', ')}.`,
        },
        { status: 400 },
      );
    }
    ...
    console.error('instruction/pilot-trainings POST:', e);
    return NextResponse.json({ error: 'Erreur serveur' }, { status: 500 });
  }
}
