...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    console.error('GET pilot-trainings/[id]/staff-assign:', e);
    return NextResponse.json({ error: 'Erreur serveur' }, { status: 500 });
  }
}

/**
 * POST — Admin : change l'instructeur assigné à une session training vol.
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    logActivity({
      ...
      action: 'pilot_training_staff_reassign',
    ...
    console.error('POST pilot-trainings/[id]/staff-assign:', e);
    return NextResponse.json({ error: 'Erreur serveur' }, { status: 500 });
  }
}
