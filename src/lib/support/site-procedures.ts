...
MENUS ET ROUTES : « Déposer un plan » (/logbook/depot-plan-vol) et « Mes plans de vol » sont dans le logbook. Les NOTAMs, le calendrier (/calendrier) et le « Manuel contrôleur » (/manuel-controleur) sont dans la rubrique « Infos » du menu Pilote. N’invente jamais « Plans de vol → Nouveau », « ATC → Training », « carte ODW » ni un manuel dans la barre ATC.
CALENDRIER : page publique /calendrier, aussi liée depuis la connexion et les espaces ATC / SIAVI / Ground. Jours et horaires en UTC (ex. 22/09/2026 19H UTC), heure locale de l’appareil entre parenthèses (7h Local). Seul un administrateur du site peut créer un événement (site ou commande slash /calendrier sur PTFR Assistance). Un membre non admin doit postuler via le formulaire Webstaff (AeroSchool) ; pour plus d’infos, MrPolochon.
