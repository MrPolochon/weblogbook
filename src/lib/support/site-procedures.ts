...
MENUS ET ROUTES : « Déposer un plan » (/logbook/depot-plan-vol) et « Mes plans de vol » sont dans le logbook. Les NOTAMs, le calendrier (/app/calendrier dans l’app connectée, /calendrier public hors shell) et le « Manuel contrôleur » (/manuel-controleur) sont dans la rubrique « Infos » du menu Pilote. N’invente jamais « Plans de vol → Nouveau », « ATC → Training », « carte ODW » ni un manuel dans la barre ATC.
CALENDRIER : page publique /calendrier (connexion / Discord). Dans l’app pilote : /app/calendrier. Espaces ATC / SIAVI / Ground ont leur miroir in-espace. Jours et horaires en UTC (ex. 22/09/2026 19H UTC), heure locale de l’appareil entre parenthèses (7h Local). Seul un administrateur du site peut créer un événement (site ou commande slash /calendrier sur PTFR Assistance). Un membre non admin doit postuler via le formulaire Webstaff (AeroSchool) ; pour plus d’infos, MrPolochon.
...
GROUND CREW : candidature sur le serveur Discord des entreprises / Ground Crew ${GROUND_CREW_DISCORD_INVITE}, pas par un QCM ni en appelant le staff. L’ATC a sa propre documentation.
SIAVI (SIAIV) : Service d’Incendie Aéronautique et d’Information en Vol, pompiers AFIS formés par le staff (formation ATC très courte). Candidature = appeler un staff, pas AeroSchool ni serveur d’entreprise.
