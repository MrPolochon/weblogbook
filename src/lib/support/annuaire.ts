 ...
 * quelqu’un hors du site), sanctions, solde Felitz, adresse IP.
 */
...
async function listInstructionStaff(admin: Admin, staff: boolean): Promise<DirectoryLookup> {
  ...
  const [profilesRes, linksRes] = await Promise.all([
    admin
      .from('profiles')
      .select('id, identifiant, role, atc, armee, ifsa, siavi, ground_crew, atc_grade_id, instruction_indisponible')
      .in('id', ids),
    admin.from('discord_links').select('user_id, discord_username').in('user_id', ids),
  ]);
  const discordById = new Map(
    (linksRes.data || []).map((row) => [String(row.user_id), String(row.discord_username || '')]),
  );

  const matches = ((profilesRes.data || []) as ProfileRow[])
    .map((profile) => ({
      identifiant: profile.identifiant || '?',
      discord: discordById.get(profile.id) || null,
  ...
  try {
    ...
    const [byIdentifiant, byDiscord] = await Promise.all([
      admin
        .from('profiles')
        .select(
          'id, identifiant, role, atc, armee, ifsa, siavi, ground_crew, atc_grade_id, instruction_indisponible',
        )
        .ilike('identifiant', like)
        .limit(MAX_RESULTS * 2),
      admin.from('discord_links').select('user_id, discord_username').ilike('discord_username', like).limit(MAX_RESULTS * 2),
    ]);

    const ids = new Set<string>();
    for (const row of (byIdentifiant.data || []) as ProfileRow[]) ids.add(row.id);
    ...
    const [profilesRes, linksRes, licencesRes] = await Promise.all([
      admin
        ...
        .in('id', idList),
      admin.from('discord_links').select('user_id, discord_username').in('user_id', idList),
