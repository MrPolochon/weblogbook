...
import PiloteMenu from './PiloteMenu';

export default function NavBar({ isAdmin, isArmee = false, pendingVolsCount = 0, volsAConfirmerCount = 0, hasCompagniePDG = false, hasCompagnie = false, showPiloteMenu = false }: { isAdmin: boolean; isArmee?: boolean; pendingVolsCount?: number; volsAConfirmerCount?: number; hasCompagniePDG?: boolean; hasCompagnie?: boolean; showPiloteMenu?: boolean }) {
    ...
    <header className="sticky top-0 z-50 border-b border-slate-700/50 bg-slate-900/95 backdrop-blur">
      <div className="mx-auto flex h-14 max-w-6xl items-center justify-between px-4">
        <nav className="flex items-center gap-1">
          {showPiloteMenu ? (
            <PiloteMenu isArmee={isArmee} hasCompagniePDG={hasCompagniePDG} hasCompagnie={hasCompagnie} />
          ) : (
            <Link
              href="/logbook"
              className={cn(
          ...
          {!showPiloteMenu && volsAConfirmerCount > 0 && (
            <Link
          ...
          {!showPiloteMenu && (isArmee || isAdmin) && (
            <Link
              href="/militaire"
              className={cn(
          ...
          {!showPiloteMenu && (
            <>
              ...
              <FelitzBankMenu hasCompagniePDG={hasCompagniePDG} />
            </>
          )}
        </nav>
