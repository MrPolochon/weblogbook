  ...
  return (
    <>
      ...
      <button
        ...
        className={`fixed top-1/3 -translate-y-1/2 z-50 flex flex-col items-center justify-center gap-1 rounded-r-xl border border-l-0 border-slate-700/50 bg-slate-900/80 px-2 py-6 hover:bg-slate-800 active:bg-slate-700 transition-all duration-300 ease-in-out shadow-xl cursor-pointer ${open ? 'left-[420px]' : 'left-0'}`}
      >
        <LayoutGrid className="h-4 w-4 text-slate-400" />
        <span className="text-[9px] font-bold text-slate-400" style={{ writingMode: 'vertical-rl' }}>
          Parkings
        </span>
        {occupied > 0 && (
      ...
      />
      <div className={`fixed left-0 top-0 h-full z-50 w-[420px] max-w-[100vw] bg-[#0a0f1c] border-r border-slate-700/50 shadow-2xl flex flex-col transition-transform duration-300 ease-in-out ${open ? 'translate-x-0' : '-translate-x-full'}`}>
            {/* Header */}
